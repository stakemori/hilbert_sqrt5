...
#+BEGIN_SRC sage
  all(check_construction(i) for i in range(3, 21))
#+END_SRC
...
* Structure 3-20
#+BEGIN_SRC sage
  [load_cand_wts(i) for i in range(3, 31)]
#+END_SRC
...
#+BEGIN_SRC sage :results table
  assert all(len(load_cand_wts(i)) == 2 for i in range(4, 31))
  wts_l = [load_cand_wts(i) for i in range(4, 31)]
  for a in wts_l:
      a[0] = list(sorted(a[0]))
      a[1] = list(sorted(a[1]))
  [(i, a[0], a[1]) for i, a in zip(range(4, 31), wts_l)]
#+END_SRC
